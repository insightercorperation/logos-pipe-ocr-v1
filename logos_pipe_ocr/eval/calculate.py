...
def calculate_testset_average_metrics(operation_results:list[dict], evaluation_metrics: dict = None) -> dict:
    final_results = {
...
def calculate_operation_average_metrics(test_data: dict, evaluation_metrics: dict) -> dict:
    overall_results = {
        "text_validation_results": {},
        "fidelity_validation_results": {
            "missing_fields": [],
            "boolean_result": {},
            "f1_score": {},
            "schema_validity_percentage": 0  # 초기화
        },
        "total_sample_size": 0
    }
    ...
    for test_name, metrics in test_data.items():
        sample_size = metrics.get("sample_size", 1)  # 기본 샘플 사이즈는 1
        overall_results["total_sample_size"] += sample_size

        # 텍스트 검증 결과 처리
        process_text_validation_results(metrics, evaluation_metrics, overall_results, test_name)

        # fidelity_validation_results 처리
        fidelity_results = metrics["fidelity_validation_results"]
        
        # missing_fields 수집
        collect_and_extend_missing_fields(overall_results["fidelity_validation_results"]["missing_fields"], fidelity_results)

        if evaluation_metrics.get("boolean"):  # boolean이 None이 아니고 비어있지 않은 경우
            if test_name in evaluation_metrics["boolean"]:  # boolean 검증 항목이 있으면 boolean_result 합산
                boolean_result = fidelity_results.get("boolean_result", {})
                calculate_overall_boolean_results(boolean_result, overall_results)

        # schema_validity_percentage 가중치 적용
        schema_validity_percentage = fidelity_results.get("schema_validity_percentage", 0)  # 기존 값 가져오기
        overall_results["fidelity_validation_results"]["schema_validity_percentage"] += schema_validity_percentage

    if overall_results["fidelity_validation_results"]["boolean_result"]: # boolean_result가 존재하면 F1 점수 계산
        calculate_f1_score(overall_results["fidelity_validation_results"]["boolean_result"], overall_results)
